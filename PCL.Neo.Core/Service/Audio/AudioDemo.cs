...
public static class AudioDemo
{
    ...
    {
        // 创建音频服务
        var audioService = AudioServiceFactory.CreateForCurrentPlatform();

        Console.WriteLine("开始播放音频...");
        var result = await audioService.PlayAsync(audioFilePath);
    ...
    public static async Task ExtensionsDemo(string audioFilePath)
    {
        ...
        var result = await audioService.PlaySoundAndWaitAsync(audioFilePath, 10000); // 10秒超时
    ...
    public static async Task StreamPlaybackDemo(string audioFilePath)
    {
        ...
        using (var stream = File.OpenRead(audioFilePath))
        {
            ...
            var result = await audioService.PlayAsync(stream, extension);
